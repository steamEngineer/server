...
def build_queue_server(  # noqa: PLR0915 -- one sub-server registers all queue tools
    ...
    def _queue_brief(queue_id: str, include_items: int) -> QueueBrief:
        ...
        return to_brief_queue(queue, items=list(items), items_offset=0)
    ...
    async def get_active_queue(
        ...
        expect — it is distinct from ``player_id``. For a queue fed by an external
        plugin source (Connect / AirPlay / Ynison), the current item's ``name`` is
